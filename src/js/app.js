	...
	if(location.pathname === '/') {
		...
		const validateForm = (button) => {
			...
			const validateInput = (input) => {
				...
				} else if(/^(8|\+7)[\d\-]{10}$/.test(input.value) && input.name === 'phone') {
					input.style.border = '1px solid #999';
					k++;
		...
		sectionHeader[1].style.display = 'none';
		...
		const validateForm = (button) => {
			...
			const validateInput = (input) => {
				...
				} else if(/^[a-zA-Zа-яА-Я'][a-zA-Zа-яА-Я-' ]+[a-zA-Zа-яА-Я']?$/u.test(input.value) && input.name === 'city') {
					input.style.border = '1px solid #999';
					k++;
					
					if(!arrayAddValue.includes(input.city)) {
						arrayAddValue.push(input.city);
					}
				} else {
					input.style.border = '1px solid red';
